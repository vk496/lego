...
static size_t encodeTagLength(byte *bytes, unsigned int tag, size_t len) {
    int i = 0;
    if (tag > 255) {
...
//    Serial.println("Send bind request");
    ...
    byte* trash = new byte[8], *trash_pointer = trash;

    //  byte version[3]; // INTEGER 3
    size_t version_len = encodeTagLengthINTEGER(trash, UNIV_INTEGER, 3);
    trash = trash_pointer;

    //  byte name[4]; // tag + len
    size_t name_len = encodeTagLength(trash, UNIV_OCTETSTRING, strlen(dn));
    trash = trash_pointer;

    //  byte authentication[4]; // tag + len
    size_t authentication_len = encodeTagLength(trash, CONTEXT_0, strlen(password));
    trash = trash_pointer;
    ...
    size_t bindrequest_len = encodeTagLength(trash, APPLICATION_0_CONS, bindrequest_content_len);
    trash = trash_pointer;

    //    byte messageid[6]; // tag + len(max 4) + int
    size_t messageid_len = encodeTagLengthINTEGER(trash, UNIV_INTEGER, _msgid);
    trash = trash_pointer;

    size_t message_content_len = messageid_len + bindrequest_len + bindrequest_content_len;

    //    byte ldapmessage[4]; // SEQUENCE + len
    size_t ldapmessage_len = encodeTagLength(trash, UNIV_SEQUENCE_CONS, message_content_len);
    trash = trash_pointer;

    delete trash_pointer;
    // Now write the bytes out in the actual order
    //    _client.write(ldapmessage, ldapmessage_len);
    //    Serial.println("ldapmessage: " + String(ldapmessage_len));
    //    _client.write(messageid, messageid_len); // includes value
    //    Serial.println("messageid: " + String(messageid_len));
    //    _client.write(bindrequest, bindrequest_len);
    //    Serial.println("bindrequest: " + String(bindrequest_len));
    //    _client.write(version, version_len); // includes value
    //    Serial.println("version: " + String(version_len));
    //    _client.write(name, name_len);
    //    Serial.println("name: " + String(name_len));
    //    _client.write((byte *) dn, strlen(dn));
    //    Serial.println("dn: " + String(strlen(dn)));
    //    _client.write(authentication, authentication_len);
    //    Serial.println("authentication: " + String(authentication_len));
    //    _client.write((byte *) password, strlen(password));
    //    Serial.println("password: " + String(strlen(password)));
    //    _client.flush();


    size_t bindRequestSize = ldapmessage_len
            + messageid_len
            + bindrequest_len
            + version_len
            + name_len
    ...
    byte* bindRequest = new byte[bindRequestSize];

    byte* bindRequest_bak = bindRequest;

    bindRequest += encodeTagLength(bindRequest, UNIV_SEQUENCE_CONS, message_content_len);

    bindRequest += encodeTagLengthINTEGER(bindRequest, UNIV_INTEGER, _msgid);

    bindRequest += encodeTagLength(bindRequest, APPLICATION_0_CONS, bindrequest_content_len);

    bindRequest += encodeTagLengthINTEGER(bindRequest, UNIV_INTEGER, 3);

    bindRequest += encodeTagLength(bindRequest, UNIV_OCTETSTRING, strlen(dn));

    memcpy(bindRequest, (byte *) dn, strlen(dn));
    bindRequest += strlen(dn);

    bindRequest += encodeTagLength(bindRequest, CONTEXT_0, strlen(password));

    memcpy(bindRequest, (byte *) password, strlen(password));
    bindRequest += strlen(password);

    //  Serial.println("Dynamic memory: " + String((&bindRequest  - &bindRequest_bak)));
    //  Serial.print("Dynamic memory int: ");
    //  Serial.println(String(packet_size));
    //  Serial.println((unsigned int) bindRequest_bak, HEX);
    //  Serial.println((unsigned int) bindRequest, HEX);
    //  Serial.println(((unsigned int) bindRequest - (unsigned int) bindRequest_bak));
    //  Serial.println(&bindRequest, HEX);

    //  printByteArray(bindRequest_bak, ((unsigned int) bindRequest - (unsigned int) bindRequest_bak));


    _client.write(bindRequest_bak, ((unsigned int) bindRequest - (unsigned int) bindRequest_bak));
    _client.flush();

    delete bindRequest_bak;

//    Serial.println("Wait for bind response");

    _pdutime = millis() + _timeout;
    _pdupos = 0;
    _error = -1;
...
//        Serial.println("Got bindResponse");
        ...
        //        Serial.print("Matched DN: ");
        //        Serial.println(matchedDN);
        //        Serial.print("Diagnostic Message: ");
 ...
 */
bool LDAPClient::read(const char dn[], const char uid[8]) {
    _msgid++;
    ...
    byte scope[3];
    encodeTagLengthINTEGER(scope, UNIV_ENUMERATED, 1);
    byte deref[3];
    ...
    //(&(objectClass=inetOrgPerson)(registeredAddress=00000000)(memberof=cn=rfid,ou=Servicios,dc=upm,dc=es))
    byte filter[] = {0xa0, 0x6c, 0xa3, 0x1c, 0x04, 0x0b, 0x6f, 0x62, 0x6a, 0x65, 0x63, 0x74, 0x43, 0x6c, 0x61, 0x73, 0x73, 0x04, 0x0d, 0x69, 0x6e, 0x65, 0x74, 0x4f, 0x72, 0x67, 0x50, 0x65, 0x72, 0x73, 0x6f, 0x6e, 0xa3, 0x1d, 0x04, 0x11, 0x72, 0x65, 0x67, 0x69, 0x73, 0x74, 0x65, 0x72, 0x65, 0x64, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x04, 0x08, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0xa3, 0x2d, 0x04, 0x08, 0x6d, 0x65, 0x6d, 0x62, 0x65, 0x72, 0x6f, 0x66, 0x04, 0x21, 0x63, 0x6e, 0x3d, 0x72, 0x66, 0x69, 0x64, 0x2c, 0x6f, 0x75, 0x3d, 0x53, 0x65, 0x72, 0x76, 0x69, 0x63, 0x69, 0x6f, 0x73, 0x2c, 0x64, 0x63, 0x3d, 0x75, 0x70, 0x6d, 0x2c, 0x64, 0x63, 0x3d, 0x65, 0x73};

//    printByteArray(filter + 55, 8);
    memcpy(filter + 55, uid, 8 * sizeof (byte));
//    printByteArray(filter + 55, 8);
    ...
    //    size_t searchRequestSize = ldapmessage_len
    //            + messageid_len
    //            + searchrequest_len
    //            + base_len
    //            + strlen(dn)
    ...
    //            + sizeof (attributes);
    //
    //    byte* searchRequest = new byte[searchRequestSize];
    //
    //    byte* searchRequest_bak = searchRequest;
    //
    //    searchRequest += encodeTagLength(searchRequest, UNIV_SEQUENCE_CONS, message_content_len); //ldapmessage
    //
    //    searchRequest += encodeTagLengthINTEGER(searchRequest, UNIV_INTEGER, _msgid); //messageid
    //
    //    searchRequest += encodeTagLength(searchRequest, APPLICATION_3_CONS, searchrequest_content_len); //searchrequest
    //
    //    searchRequest += encodeTagLength(searchRequest, UNIV_OCTETSTRING, strlen(dn)); //base
    //
    //    memcpy(searchRequest, (byte *) dn, strlen(dn)); //dn
    //    searchRequest += strlen(dn);
    //
    //    searchRequest += encodeTagLengthINTEGER(searchRequest, UNIV_ENUMERATED, 0); //scope
    //
    //    searchRequest += encodeTagLengthINTEGER(searchRequest, UNIV_ENUMERATED, 3); //deref
    //
    //    searchRequest += encodeTagLengthINTEGER(searchRequest, UNIV_INTEGER, 0); //sizelimit
    //
    //    searchRequest += encodeTagLengthINTEGER(searchRequest, UNIV_INTEGER, 0); //timelimit
    //
    //    searchRequest += encodeTagLengthBOOLEAN(searchRequest, UNIV_BOOLEAN, false); //typesonly
    //
    //    searchRequest += encodeTagLength(searchRequest, CONTEXT_7, strlen("objectClass")); //filter
    //
    //    strncpy((char*) searchRequest, "objectClass", strlen("objectClass")); //filter field
    //    searchRequest += strlen("objectClass");
    //
    //
    //    searchRequest += encodeTagLength(searchRequest, UNIV_SEQUENCE_CONS, 0); //attributes
    //    Serial.println("Send search query");
    //
    //
    //    //  Serial.println("Dynamic memory: " + String((&bindRequest  - &bindRequest_bak)));
    //    //  Serial.print("Dynamic memory int: ");
    //    //  Serial.println(String(packet_size));
    //    Serial.println((unsigned int) searchRequest_bak, HEX);
    //    Serial.println((unsigned int) searchRequest, HEX);
    //    Serial.println(((unsigned int) searchRequest - (unsigned int) searchRequest_bak));
    //
    //    printByteArray(searchRequest_bak, ((unsigned int) searchRequest - (unsigned int) searchRequest_bak));
    //
    //    Serial.flush();
    //
    //
    //
    //    _client.write(searchRequest_bak, ((unsigned int) searchRequest - (unsigned int) searchRequest_bak));
    //    _client.flush();
    //
    //    delete searchRequest_bak;
...
//                            Serial.println("Value read");
                        ...
                        if (attribute.equals("registeredAddress")) {
                            if (value.equals(uid)) {
                                return_value = true;
                            }
...
//void LDAPClient::printByteArray(byte *buffer, int length) {
